...
# 6. Model Building for Prediction of Smoking Status:

# Choose a classification model - Random Forest
# Example: Random Forest for predicting smoking status
forest_model <- randomForest(smoking ~ age + gender + weight.kg. + height.cm. + waist.cm. + eyesight.left. + hearing.left. + hearing.right. + systolic + relaxation + fasting.blood.sugar + Cholesterol + triglyceride + HDL + LDL + hemoglobin + Urine.protein + serum.creatinine + AST + ALT + Gtp + oral + dental.caries + tartar , data = data)

# Justify the choice of the model
# In this case, Random Forest is chosen for its ability to handle non-linear relationships and interactions.

# Print model summary
print(forest_model)

# Convert 'smoking' to numeric
data$smoking_numeric <- as.numeric(data$smoking) - 1  # Assuming 0 for 'non-smoking' and 1 for 'smoking'

# Assess model performance
predicted_probabilities <- predict(forest_model, data, type = "response")
predicted_labels <- ifelse(as.numeric(predicted_probabilities) > 0.5, "smoking", "non-smoking")

print(predicted_probabilities)
# Create confusion matrix
conf_matrix <- table(data$smoking_numeric, predicted_labels)
print(conf_matrix)
